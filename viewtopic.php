 ...
 *
 * Note:
 *      + Aggiunta la Quick Reply Mod
 *      + Aggiunta la Karma Mod
 *
 ***************************************************************************/ 
...
//
// Modding for Karma Mod By KinG-InFeT
//
...
for($i = 0; $i < $total_posts; $i++)
{
	...
	//
	//Karma Mod By BlackLight Modded By KinG-InFeT for Salviamo-phpBB2 Project
	//
	
	if (intval($postrow[$i]['user_karma']) > 0)
    	$postrow[$i]['user_karma'] = '<b><font color="#119911">'.$postrow[$i]['user_karma'].'</b></font>'.'<br><a href="get_karma.php?voter='.htmlspecialchars(crypt($userdata['user_id'],"00")).'&voted='.$postrow[$i]['user_id'].'&vote=plus">Positivo</a>/<a href="get_karma.php?voter='.htmlspecialchars(crypt($userdata['user_id'],"00")).'&voted='.$postrow[$i]['user_id'].'&vote=minus">Negativo</a>';
    elseif (intval($postrow[$i]['user_karma']) < 0)
    	$postrow[$i]['user_karma'] = '<b><font color="#991111">'.$postrow[$i]['user_karma'].'</b></font>'.'<br><a href="get_karma.php?voter='.htmlspecialchars(crypt($userdata['user_id'],"00")).'&voted='.$postrow[$i]['user_id'].'&vote=plus">Positivo</a>/<a href="get_karma.php?voter='.htmlspecialchars(crypt($userdata['user_id'],"00")).'&voted='.$postrow[$i]['user_id'].'&vote=minus">Negativo</a>';
    else
    	$postrow[$i]['user_karma'] = '<b><font color="#888888">'.$postrow[$i]['user_karma'].'</b></font>'.'<br><a href="get_karma.php?voter='.htmlspecialchars(crypt($userdata['user_id'],"00")).'&voted='.$postrow[$i]['user_id'].'&vote=plus">Positivo</a>/<a href="get_karma.php?voter='.htmlspecialchars(crypt($userdata['user_id'],"00")).'&voted='.$postrow[$i]['user_id'].'&vote=minus">Negativo</a>';
    
    $poster_karma = ( ($postrow[$i]['user_karma'] != NULL) && ($postrow[$i]['user_id'] != ANONYMOUS )) ? "<br>Reputazione: ".$postrow[$i]['user_karma'] : "";
...
else
{
	...
	$quick_reply_form .= "<textarea name='input' rows='17' cols='110' wrap='virtual' class='post''></textarea><br />
