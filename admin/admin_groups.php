...
else if ( isset($HTTP_POST_VARS['group_update']) )
{
	...
	else
	{
		...
		$group_description = isset($HTTP_POST_VARS['group_description']) ? htmlspecialchars(trim($HTTP_POST_VARS['group_description'])) : '';
		$group_moderator = isset($HTTP_POST_VARS['username']) ? htmlspecialchars($HTTP_POST_VARS['username']) : '';
...
else
{
	...
	if ( $row = $db->sql_fetchrow($result) )
	{
		...
		do
		{
			$select_list .= '<option value="' . $row['group_id'] . '">' . htmlspecialchars($row['group_name']) . '</option>';
		}
