...
if( empty($forum_id) )
{
	...
	{
		$select_list .= '<option value="' . (int) $forum_rows[$i]['forum_id'] . '">' . htmlspecialchars($forum_rows[$i]['forum_name']) . '</option>';
	}
	$select_list .= '</select>';
...
else
{
	...
	$forum_name = htmlspecialchars($forum_rows[0]['forum_name']);
	...
	if ( empty($adv) )
	{
		...
		for($j = 0; $j < count($simple_auth_types); $j++)
		{
			...
			$simple_auth .= '<option value="' . $j . '"' . $selected . '>' . htmlspecialchars($simple_auth_types[$j]) . '</option>';
		}
	...
	else
	{
		...
		for($j = 0; $j < count($forum_auth_fields); $j++)
		{
			...
			for($k = 0; $k < count($forum_auth_levels); $k++)
			{
				...
				$custom_auth[$j] .= '<option value="' . $forum_auth_const[$k] . '"' . $selected . '>' . htmlspecialchars($lang['Forum_' . $forum_auth_levels[$k]]) . '</option>';
			}
	...
	$s_hidden_fields = '<input type="hidden" name="' . POST_FORUM_URL . '" value="' . (int) $forum_id . '">';
