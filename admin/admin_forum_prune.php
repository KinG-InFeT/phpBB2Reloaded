...
else
{
	...
	if( empty($HTTP_POST_VARS[POST_FORUM_URL]) )
	{
		...
		$select_list .= '<option value="-1">' . htmlspecialchars($lang['All_Forums']) . '</option>';

		for($i = 0; $i < count($forum_rows); $i++)
		{
			$select_list .= '<option value="' . $forum_rows[$i]['forum_id'] . '">' . htmlspecialchars($forum_rows[$i]['forum_name']) . '</option>';
		}
		$select_list .= '</select>';
