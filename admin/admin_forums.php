...
if( isset($HTTP_POST_VARS['addforum']) || isset($HTTP_POST_VARS['addcategory']) )
{
	...
	if( $mode == "addforum" )
	{
		...
		//
		$forumname = htmlentities(stripslashes($HTTP_POST_VARS['forumname'][$cat_id]));
	}
}
	...
	switch($mode)
	{
		...
		case 'editforum':
			...
			if ($mode == 'editforum')
			{
				...
				$cat_id = $row['cat_id'];
				$forumname = htmlentities($row['forum_name']);
				$forumdesc = htmlentities($row['forum_desc']);
				$forumstatus = htmlentities($row['forum_status']);
			...
			$template->assign_vars(array(
				...
				'FORUM_NAME' => htmlentities($forumname),
				'DESCRIPTION' => htmlentities($forumdesc))
			);
		...
		case 'createforum':
			...
			$sql = "INSERT INTO " . FORUMS_TABLE . " (forum_id, forum_name, cat_id, forum_desc, forum_order, forum_status, prune_enable" . $field_sql . ")
				VALUES ('" . $next_id . "', '" . htmlentities(str_replace("\'", "''", $HTTP_POST_VARS['forumname'])) . "', " . intval($HTTP_POST_VARS[POST_CAT_URL]) . ", '" . htmlentities(str_replace("\'", "''", $HTTP_POST_VARS['forumdesc'])) . "', $next_order, " . intval($HTTP_POST_VARS['forumstatus']) . ", " . intval($HTTP_POST_VARS['prune_enable']) . $value_sql . ")";
			if( !$result = $db->sql_query($sql) )
			{
		...
		case 'modforum':
			...
			$sql = "UPDATE " . FORUMS_TABLE . "
				SET forum_name = '" . htmlentities(str_replace("\'", "''", $HTTP_POST_VARS['forumname'])) . "', cat_id = " . intval($HTTP_POST_VARS[POST_CAT_URL]) . ", forum_desc = '" . htmlentities(str_replace("\'", "''", $HTTP_POST_VARS['forumdesc'])) . "', forum_status = " . intval($HTTP_POST_VARS['forumstatus']) . ", prune_enable = " . intval($HTTP_POST_VARS['prune_enable']) . "
		...
		case 'addcat':
			...
			$sql = "INSERT INTO " . CATEGORIES_TABLE . " (cat_title, cat_order)
				VALUES ('" . htmlentities(str_replace("\'", "''", $HTTP_POST_VARS['categoryname'])) . "', $next_order)";
			if( !$result = $db->sql_query($sql) )
			{
		...
		case 'modcat':
			...
			$sql = "UPDATE " . CATEGORIES_TABLE . "
				SET cat_title = '" . htmlentities(str_replace("\'", "''", $HTTP_POST_VARS['cat_title'])) . "'
				WHERE cat_id = " . intval($HTTP_POST_VARS[POST_CAT_URL]);
			if( !$result = $db->sql_query($sql) )
			{
				message_die(GENERAL_ERROR, "Couldn't update forum information", "", __LINE__, __FILE__, $sql);
		...
		for($j = 0; $j < $total_forums; $j++)
		{
			...
			if ($forum_rows[$j]['cat_id'] == $cat_id)
			{
				...
				$template->assign_block_vars("catrow.forumrow",	array(
					'FORUM_NAME' => htmlspecialchars($forum_rows[$j]['forum_name']),
					'FORUM_DESC' => htmlspecialchars($forum_rows[$j]['forum_desc']),
					'ROW_COLOR' => @$row_color,
