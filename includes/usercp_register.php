...
if ( isset($HTTP_POST_VARS['submit']) )
{
	...
	else if ( $mode == 'register' )
	{
		...
		//New Control of Captcha By KinG-InFeT
		session_start();
		if (!isset($_POST['captcha']) || 
		    !isset($_SESSION['freecap_word_hash']) || 
		    $_SESSION['hash_func']
		    ($_POST['captcha']) != $_SESSION['freecap_word_hash'])
		{
			$error = TRUE;
	...
	if ( !$error )
	{
		...
		else
		{
			...
			if( $coppa )
			{
			    $board_config['sitename'] = htmlspecialchars($board_config['sitename']);
				$emailer->assign_vars(array(
					'SITENAME' => $board_config['sitename'],
...
else
{
	...
	$template->assign_vars(array(
		...
		//'TOKEN' => $_REQUEST['token'], // magic token
		'YIM' => $yim,
		'ICQ' => $icq,
		'MSN' => $msn,
		'AIM' => $aim,
