...
if ( isset($HTTP_POST_VARS['search_keywords']) || isset($HTTP_GET_VARS['search_keywords']) )
{
	...
	*/
	$search_keywords = mysql_real_escape_string( htmlentities( $search_keywords));
}
else
{
	$search_keywords = '';
...
if ( isset($HTTP_POST_VARS['search_author']) || isset($HTTP_GET_VARS['search_author']))
{
	...
	# Fix SQL Injection and XSS
	*/
	$search_author = mysql_real_escape_string( htmlentities( $search_author));
...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' )
	{
		...
		*/
		...
		{
		    //Very fix in code :S By KinG-InFeT
	...
	if ( $search_results != '' )
	{
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			else
			{
				...
				$template->assign_block_vars('searchresults', array( 
					'FORUM_NAME' => htmlspecialchars($searchset[$i]['forum_name']),
...
while( $row = $db->sql_fetchrow($result) )
{
	...
	{
		$s_forums .= '<option value="' . htmlspecialchars($row['forum_id']) . '">' . htmlspecialchars($row['forum_name']) . '</option>';
...
if ( $s_forums != '' )
{
	...
	{
		$s_categories .= '<option value="' . htmlspecialchars($cat_id) . '">' . htmlspecialchars($cat_title) . '</option>';
	}
}
else
{
